// hi sorry for bad formatting and stuff and unreadable https://beautifier.io/
...
function mainLoop() {
    ...
    for (let y = 0; y < mapHeight; y++) drawWall(0, y, 'left');
    for (let x = 0; x < mapWidth; x++) drawWall(x, 0, 'back');
    ...
    if (highlightObject) {
        ...
        if (editor.currentTool === 'place') highlightObject.draw(ctx, screenWidth / 2, 117, tileHeight, gridToIso);
        ctx.globalAlpha = 1.0;
...
const editor = {
    ...
    setupEventListeners() {
        ...
        canvas.addEventListener('mousedown', (e) => editor.startDrag(e));
        canvas.addEventListener('mousemove', (e) => editor.dragObject(e));
        canvas.addEventListener('mouseup', (e) => editor.stopDrag(e));
    },
    setupColorPicker(id) {
    ...
    deleteObjectAtPosition(gridX, gridY) {
        ...
        if (obj) this.deleteObject(obj.id);
    },
    clearAllObjects() {
